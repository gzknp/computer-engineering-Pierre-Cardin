   ...
   public class CartViewModel:BaseViewModel
    {
        ...
        public Command GoToOdeme { get; set; }
        public CartViewModel()
        {
            CartItems = new ObservableCollection<UserCartItem>();
            LoadItems();
            PlaceOrdersCommand = new Command(async () => await PlaceOrdersAsync());
            GoToOdeme = new Command(async () => await GoToOdemeAsync());
        }
        private async Task GoToOdemeAsync()
        {
            var id =  new OdemeService();
            await Application.Current.MainPage.Navigation.PushAsync(new OdemeView());
        }
        private async Task PlaceOrdersAsync()
        {
            var id = await new OrderService().PlaceOrderAsync() as string;
            RemoveItemsFromCart();
            await Application.Current.MainPage.Navigation.PushAsync(new OrderView(id));
        }
